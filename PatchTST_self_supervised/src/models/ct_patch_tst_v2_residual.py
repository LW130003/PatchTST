...
The model is already parameter heavy, so we don't implement adapter. 

Let's try adding residual first. 
...
class TSTEncoder(nn.Module):
    def __init__(self, d_model, n_heads, d_ff=None, 
        ...
        self.layers = nn.ModuleList([
            ChannelTimeTSTEncoderLayer(d_model, n_heads=n_heads, d_ff=d_ff, norm=norm,
    ...
    def forward(self, src:Tensor, shape: Tuple[int, int, int, int]):
        """
...
class ChannelTimeTSTEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, n_heads, d_ff=256, store_attn=False,
        ...
        self.time_attention = TSTEncoderLayer(
            d_model, n_heads=n_heads, d_ff=d_ff, norm=norm,
            attn_dropout=attn_dropout, dropout=dropout,
            activation=activation, res_attention=res_attention,
            pre_norm=pre_norm, store_attn=store_attn            
...
class ChannelTSTEncoderLayer(nn.Module):
    ...
    def forward(self, src:Tensor, prev:Optional[Tensor]=None, 
        ...
        src = self._reshape_to_mix_channel(src, shape)
        ...
        # reshape at the end
        # attention: information gatherer.
        # FFN: knowledge processor. 
        src = self._reshape_to_channel_independent(src, shape)            
